...
public class QRGenerator {
    ...
    public QRGenerator(String data, BarcodeFormat format, int width, int height, int imageType) throws WriterException {
        ...
        int yellowColor = 0xFFC0C0C0; // Yellow background
    ...
    private void qrImageCreate(int width, int height, int imageType, int foregroundColor, int backgroundColor) {
        this.qrImage = new BufferedImage(width, height, imageType);
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                qrImage.setRGB(x, y, this.bitMatrix.get(x, y) ? foregroundColor : backgroundColor);
            }
        }
    }
